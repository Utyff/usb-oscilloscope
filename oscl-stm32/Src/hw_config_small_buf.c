...
uint32_t  active_ch_nums=2;
bool      CH_active[MAX_ADC_CH]={TRUE, TRUE, FALSE, FALSE};
...
//   DMA_ITConfig( DMA1_Channel1, DMA1_IT_TC1, DISABLE );
   ...
   buf[to_fill][fill_count++]= ADC_Values[1];
...
#define CCR_CLEAR_Mask           ((uint32_t)0xFFFF800F)
  ...
  tmpreg |= DMA_DIR_PeripheralSRC | DMA_Mode_Circular | DMA_PeripheralInc_Disable | DMA_MemoryInc_Enable |
...
//    while( !(ADC1->CR2 & ADC_CR2_RSTCAL) ); // wait for reset end
    ...
    // while ( !(ADC1->SR & ADC_SR_EOC) ); // wait for first conversion
}
